...
#+cormanlisp
(export '(loop loop-finish loop-tequal loop-tassoc loop-tmember
		named-variable loop-lookup-keyword loop-store-table-data))
...
(defmacro loop-collect-rplacd ((head-var tail-var &optional user-head-var) form &aux env) ;; &environment
  (declare
    #+LISPM (ignore head-var user-head-var)	;use locatives, unconditionally update through the tail.
    )
  (setq form (macroexpand form env))
  (flet ((cdr-wrap (form n)
	   #-cormanlisp (declare (fixnum n))
...
(defmacro loop-really-desetq (&rest var-val-pairs &aux env ) ;; &environment
...
(defmacro loop-body (
		     ...
		     after-loop
		     epilogue
		     &aux rbefore rafter flagvar (env nil))
...
;;;INTERFACE: ANSI
(defmacro cl::loop (&rest keywords-and-forms ) ;; &environment
  #+Genera (declare (compiler:do-not-record-macroexpansions)
		    (zwei:indentation . zwei:indent-loop))
  (loop::loop-standard-expansion keywords-and-forms nil *loop-ansi-universe*))

...
(shadow '(write print prin1 princ pprint format write-to-string princ-to-string
	  ...
	  defstruct finish-output force-output clear-output streamp))

;; save these functions away for use by the functions in this module
(setf (symbol-function 'lisp+write) #'cl::write)
(setf (symbol-function 'lisp+print) #'cl::print)
(setf (symbol-function 'lisp+prin1) #'cl::prin1)
(setf (symbol-function 'lisp+princ) #'cl::princ)
(setf (symbol-function 'lisp+pprint) #'cl::pprint)
(setf (symbol-function 'lisp+format) #'cl::format)
(setf (symbol-function 'lisp+write-to-string) #'cl::write-to-string)
(setf (symbol-function 'lisp+princ-to-string) #'cl::princ-to-string)
(setf (symbol-function 'lisp+prin1-to-string) #'cl::prin1-to-string)
(setf (symbol-function 'lisp+write-line) #'cl::write-line)
(setf (symbol-function 'lisp+write-string) #'cl::write-string)
(setf (symbol-function 'lisp+write-char) #'cl::write-char)
(setf (symbol-function 'lisp+terpri) #'cl::terpri)
(setf (symbol-function 'lisp+fresh-line) #'cl::fresh-line)
(setf (macro-function 'lisp+defstruct) (macro-function 'cl::defstruct))
(setf (symbol-function 'lisp+finish-output) #'cl::finish-output)
(setf (symbol-function 'lisp+force-output) #'cl::force-output)
(setf (symbol-function 'lisp+clear-output) #'cl::clear-output)
(setf (symbol-function 'lisp+streamp) #'cl::streamp)
...
;;; RGC: changed this from defvar to defparameter because the
;;; variable already has a value (from the kernel) before this is loaded
;;; and defvar does not pick up the change here
(defparameter *print-pprint-dispatch* t ;see initialization at end of file.
...
#+:cormanlisp (eval-when (eval load compile)
		(setf cl::*compiler-warn-on-unused-variable* nil)
		(setf cl::*compiler-warn-on-undefined-function* nil)
		(setf cl::*undefined-functions* t)
		(setf cl::*compiler-warn-on-assumed-special* nil))

#+:cormanlisp (eval-when (eval load compile)
		(defun structure-type-p (x) (and (symbolp x) (get x ':STRUCT-TEMPLATE)))
		(defun output-width     (&optional (s *standard-output*)) (declare (ignore s)) nil)
		(defun output-position  (&optional (s *standard-output*)) (cl::stream-col-position s)) )
...
(defun format (stream string-or-fn &rest args)
  (cond ((stringp stream)
	 (lisp+format stream "~A"
		      (with-output-to-string (stream)
			(apply #'xp::format stream string-or-fn args)))
	 nil)
	((null stream)
	 (with-output-to-string (stream)
	   (apply #'xp::format stream string-or-fn args)))
...
;; RGC (eval-when (eval load compile)
...
(def-format-handler #\? (start end) (declare (ignore end))
		    (multiple-value-bind (colon atsign) (parse-params start nil :nocolon t)
		      ...
		      (if (not atsign) `(apply #'xp::format xp ,(get-arg) ,(get-arg))
...
#+:cormanlisp (eval-when (eval load)
		...
		(defun comma-dot-print (xp list)
		  (if (and (consp (cdr list)) (null (cddr list)))
		      (funcall (formatter ",.~W") xp (cadr list))
		      (pprint-fill xp list)))
		(set-pprint-dispatch+ '(cons (member cl::backquote)) #'backquote-print '(0) *IPD*)
		(set-pprint-dispatch+ '(cons (member cl::%comma)) #'comma-print '(0) *IPD*)
		(set-pprint-dispatch+ '(cons (member cl::%comma-atsign)) #'comma-atsign-print '(0) *IPD*)
		(set-pprint-dispatch+ '(cons (member cl::%comma-dot)) #'comma-dot-print '(0) *IPD*)
		)
...
(defun pprint-dispatch-print (xp table)
  (let ((stuff (copy-list (others table))))
    ...
    (pprint-logical-block (xp stuff :prefix "#<" :suffix ">")
      (xp::format xp (formatter "pprint dispatch table containing ~A entries: ")
		  (length stuff))
      (loop (pprint-exit-if-list-exhausted)
       (let ((entry (pprint-pop)))
	 (xp::format xp (formatter "~{~_P=~4D ~W~} F=~W ")
...
(setf (symbol-function 'cl::write) #'xp::write)
(setf (symbol-function 'cl::print) #'xp::print)
(setf (symbol-function 'cl::prin1) #'xp::prin1)
(setf (symbol-function 'cl::princ) #'xp::princ)
(setf (symbol-function 'cl::pprint) #'xp::pprint)
(setf (symbol-function 'cl::format) #'xp::format)
(setf (symbol-function 'cl::write-to-string) #'xp::write-to-string)
(setf (symbol-function 'cl::princ-to-string) #'xp::princ-to-string)
(setf (symbol-function 'cl::prin1-to-string) #'xp::prin1-to-string)
(setf (symbol-function 'cl::write-line) #'xp::write-line)
(setf (symbol-function 'cl::write-string) #'xp::write-string)
(setf (symbol-function 'cl::write-char) #'xp::write-char)
(setf (symbol-function 'cl::terpri) #'xp::terpri)
(setf (symbol-function 'cl::fresh-line) #'xp::fresh-line)
(setf (macro-function 'cl::defstruct) (macro-function 'xp::defstruct))
(setf (symbol-function 'cl::finish-output) #'xp::finish-output)
(setf (symbol-function 'cl::force-output) #'xp::force-output)
(setf (symbol-function 'cl::clear-output) #'xp::clear-output)
(setf (symbol-function 'cl::streamp) #'xp::streamp)

#+:cormanlisp (eval-when (eval load compile)
		(setf cl::*compiler-warn-on-unused-variable* t)
		(setf cl::*compiler-warn-on-undefined-function* t)

...
#define TEST \
	0 // set this to 1 to compile a test console app
...
CString EncryptDES::DecryptCString(const CString& key, const CString& hex)
{
	...
	CString s;
	des.importHex(hex);
	int decryptedLength = des.decrypt(key, (LPBYTE)s.GetBuffer(des.getBinaryLength()), des.getBinaryLength());
	s.ReleaseBuffer(decryptedLength);
	return s;
}

int EncryptDES::getBinaryLength()
{
	return bufSize;
}

int EncryptDES::getHexStringLength()
{
	return bufSize * 2;
}
...
void EncryptDES::processKey(const char* key, bool decipher)
{
...
{
#ifdef _M_IX86
	__asm mov eax, dword ptr src __asm mov edx, dword ptr dest __asm mov ecx,
		dword ptr[eax] __asm bswap ecx __asm mov dword ptr[edx], ecx __asm mov ecx,
		dword ptr[eax + 4] __asm bswap ecx __asm mov dword ptr[edx + 4], ecx
#else
...
//
const unsigned char EncryptDES::pc1[56] = {56, 48, 40, 32, 24, 16, 8,  0,  57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42,
										   34, 26, 18, 10, 2,  59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6,  61, 53,
										   45, 37, 29, 21, 13, 5,  60, 52, 44, 36, 28, 20, 12, 4,  27, 19, 11, 3};

const unsigned char EncryptDES::pc2[48] = {13, 16, 10, 23, 0,  4,  2,  27, 14, 5,  20, 9,  22, 18, 11, 3,
										   25, 7,  15, 6,  26, 19, 12, 1,  40, 51, 30, 36, 46, 54, 29, 39,
										   50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31};

const unsigned short EncryptDES::bytebit[8] = {0200, 0100, 040, 020, 010, 04, 02, 01};

const unsigned char EncryptDES::totrot[16] = {1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28};

const unsigned long EncryptDES::bigbyte[24] = {0x800000L, 0x400000L, 0x200000L, 0x100000L, 0x80000L, 0x40000L,
...
void usage()
{
	printf("Usage: encryptdes key string\n"

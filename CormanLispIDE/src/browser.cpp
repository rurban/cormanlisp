...
int CBrowserFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndToolBar.Create(this) || !m_wndToolBar.LoadToolBar(IDR_BROWSERFRAME))
	{
	...
	if (!m_cbURL.CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), 0, WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL | WS_BORDER, rect,
						  &m_wndToolBar, IDC_URL_NAME))
	{
...
BOOL CBrowserFrame::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == 1 && lParam == 0)
	{
		...
		try
		{
			((CBrowserView*)GetActiveView())->m_WebBrowser.Navigate(strUrl, &varEmpty, &varEmpty, &varEmpty, &varEmpty);
		}
		catch (CException* excp)
		{
			excp->Delete();
...
BOOL CBrowserFrame::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		if (m_cbURL.m_hWnd != pMsg->hwnd || pMsg->wParam == VK_TAB)
		{
			LPDISPATCH lpDispatch = NULL;

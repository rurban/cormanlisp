...
const long FirstSpecialSymbolEntry = FirstJumpTableEntry + (NumJumpTableEntries * JumpTableCellsPerEntry);
...
#define arrayDimension(a, dim) (isAdjustableArray(a) ? (adjustableArrayLength(a, dim)) : simpleVectorLength(a))
...
#define vectorLength(a)                                     \
	(isAdjustableArray(a) ? (UVECTOR(a)[ARRAY_DIM1_LENGTH]) \
...
#define isGenericArray(a) \
	(isUvector(a) &&      \
	 (isSimpleGenericVector(a) || (isAdjustableArray(a) && isSimpleGenericVector(adjustableArrayVector(a)))))
#define isString(a) \
	(isVector(a) && (isSimpleCharVector(a) || (isAdjustableArray(a) && isSimpleCharVector(adjustableArrayVector(a)))))
#define isBitVector(a) \
	(isVector(a) && (isSimpleBitVector(a) || (isAdjustableArray(a) && isSimpleBitVector(adjustableArrayVector(a)))))
#define isByteVector(a) \
	(isVector(a) && (isSimpleByteVector(a) || (isAdjustableArray(a) && isSimpleByteVector(adjustableArrayVector(a)))))
#define isCharArray(a) \
	(isArray(a) && (isSimpleCharVector(a) || (isAdjustableArray(a) && isSimpleCharVector(adjustableArrayVector(a)))))
#define isByteArray(a) \
	(isArray(a) && (isSimpleByteVector(a) || (isAdjustableArray(a) && isSimpleByteVector(adjustableArrayVector(a)))))
#define isBitArray(a) \
	(isArray(a) && (isSimpleBitVector(a) || (isAdjustableArray(a) && isSimpleBitVector(adjustableArrayVector(a)))))
...
#define isHashtable(n) \
	(isStruct(n) && isSimpleVector(structTemplate(n)) && (simpleVectorStart(structTemplate(n))[0] == HASH_TABLE))
...
#define GetArgCount(var) __asm mov var, ecx

#define ReturnCount(num) __asm mov ecx, num
...
#define LISP_FUNC_BEGIN(numargs)                      \
	LispObj ret = 0;                                  \
	long _arg_count;                                  \
	asm volatile("mov %%ecx, %0" : "=r"(_arg_count)); \
	LispObj* _arg_ptr = &_args;                       \
	_arg_ptr += (_arg_count - 1);                     \
	CheckNumArgs(numargs)
...
#define LISP_FUNC_RETURN(val)                       \
	...
	{                                               \
		...
		asm volatile("movl $1, %0" : "=c"(_dummy)); \
		return ret;                                 \
	} while (0)
#define LISP_FUNC_RETURN_NO_VALUES()                \
	do                                              \
	{                                               \
		ret = NIL;                                  \
		unsigned long _dummy;                       \
		asm volatile("movl $0, %0" : "=c"(_dummy)); \
		return ret;                                 \
	} while (0)
#define LISP_TO_FOREIGN() \
	do                    \
...
public:
	LispHeap();
	~LispHeap();
	void alloc(long unsigned size, unsigned long overflow, unsigned long generation, unsigned long reserveRequested);

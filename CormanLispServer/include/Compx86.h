...
#define CMP_EAX_ESP_PTR_WITH_OFFSET(offset) \
	...
	{                                       \
		ASM_OP24(0x3b, 0x44, 0x24);         \
		ASM_BYTE(offset);                   \
	}                                       \
	else                                    \
	{                                       \
		ASM_OP24(0x3b, 0x84, 0x24);         \
		ASM_LONG(offset);                   \
	}
...
#define MOV_EDI_ESP_PTR_WITH_OFFSET(offset) \
	...
	{                                       \
		ASM_OP24(0x8b, 0x7c, 0x24);         \
		ASM_BYTE(offset);                   \
	}                                       \
	else                                    \
	{                                       \
		ASM_OP24(0x8b, 0xbc, 0x24);         \
		ASM_LONG(offset);                   \
	}
...
#define MOV_EAX_EAX_PLUS_EDX_PTR_WITH_OFFSET(offset) \
	...
	{                                                \
		ASM_OP24(0x8b, 0x44, 0x10);                  \
		ASM_BYTE(offset);                            \
	}                                                \
	else                                             \
	{                                                \
		ASM_OP24(0x8b, 0x84, 0x10);                  \
		ASM_LONG(offset);                            \
	}
...
#define MOV_EDI_PLUS_EDX_PTR_WITH_OFFSET_EAX(offset) \
	...
	{                                                \
		ASM_OP24(0x89, 0x44, 0x17);                  \
		ASM_BYTE(offset);                            \
	}                                                \
	else                                             \
	{                                                \
		ASM_OP24(0x89, 0x84, 0x17);                  \
		ASM_LONG(offset);                            \
	}
...
#define JLE_RELATIVE(offset)  \
	{                         \
		ASM_OP16(0x0f, 0x8e); \
		ASM_LONG(offset);     \
	}
#define JL_RELATIVE(offset)   \
	{                         \
		ASM_OP16(0x0f, 0x8c); \
		ASM_LONG(offset);     \
	}
#define JG_RELATIVE(offset)   \
	{                         \
		ASM_OP16(0x0f, 0x8f); \
		ASM_LONG(offset);     \
	}
#define JGE_RELATIVE(offset)  \
	{                         \
		ASM_OP16(0x0f, 0x8d); \
		ASM_LONG(offset);     \
	}
#define JZ_RELATIVE(offset)   \
	{                         \
		ASM_OP16(0x0f, 0x84); \
		ASM_LONG(offset);     \
	}
#define JNE_RELATIVE(offset)  \
	{                         \
		ASM_OP16(0x0f, 0x85); \
		ASM_LONG(offset);     \
	}
#define JNO_RELATIVE(offset)  \
	{                         \
		ASM_OP16(0x0f, 0x81); \
		ASM_LONG(offset);     \
	}
...
#define JLE_SHORT_RELATIVE(offset) \
	{                              \
		ASM_OP8(0x7e);             \
		ASM_BYTE(offset);          \
	}
#define JL_SHORT_RELATIVE(offset) \
	{                             \
		ASM_OP8(0x7c);            \
		ASM_BYTE(offset);         \
	}
#define JG_SHORT_RELATIVE(offset) \
	{                             \
		ASM_OP8(0x7f);            \
		ASM_BYTE(offset);         \
	}
#define JGE_SHORT_RELATIVE(offset) \
	{                              \
		ASM_OP8(0x7d);             \
		ASM_BYTE(offset);          \
	}
#define JZ_SHORT_RELATIVE(offset) \
	{                             \
		ASM_OP8(0x74);            \
		ASM_BYTE(offset);         \
	}
#define JNE_SHORT_RELATIVE(offset) \
	{                              \
		ASM_OP8(0x75);             \
		ASM_BYTE(offset);          \
	}
#define JNO_SHORT_RELATIVE(offset) \
	{                              \
		ASM_OP8(0x71);             \
		ASM_BYTE(offset);          \
	}
...
#define JMP_SHORT_RELATIVE(offset) \
	{                              \
		ASM_OP8(0xeb);             \
		ASM_BYTE(offset);          \
	}

#define JMP_LONG_RELATIVE(offset) \
	{                             \
		ASM_OP8(0xe9);            \
		ASM_LONG(offset);         \
	}
...
#define PUSH_EIP_AND_JMP(offset) \
	{                            \
		CALL_RELATIVE(offset);   \
		INC_STACK(4);            \
	}
...
#define MOV_EDX_NUM(num) \
	{                    \
		ASM_OP8(0xba);   \
		ASM_LONG(num);   \
	} // mov edx, 0x1234
#define MOV_EDI_NUM(num) \
	{                    \
		ASM_OP8(0xbf);   \
		ASM_LONG(num);   \
	} // mov edi, 0x1234
...
#define SHL_EAX_NUM(num)      \
	{                         \
		ASM_OP16(0xc1, 0xe0); \
		ASM_BYTE(num);        \
	} // shl eax, 2
#define SHL_ECX_NUM(num)      \
	{                         \
		ASM_OP16(0xc1, 0xe1); \
		ASM_BYTE(num);        \
	} // shl ecx, 2
#define SHR_EAX_NUM(num)      \
	{                         \
		ASM_OP16(0xc1, 0xe8); \
		ASM_BYTE(num);        \
	} // shr eax, 2
#define SHR_EDX_NUM(num)      \
	{                         \
		ASM_OP16(0xc1, 0xea); \
		ASM_BYTE(num);        \
	} // shr edx, 2
...
#define PUSH_EDI_EBX_WITH_OFFSET(offset) \
	{                                    \
		...
		{                                \
			ASM_OP24(0xff, 0x74, 0x1f);  \
			ASM_BYTE(offset);            \
		}                                \
		else                             \
		{                                \
			ASM_OP24(0xff, 0xb4, 0x1f);  \
			ASM_LONG(offset);            \
		}                                \
	}
...
#define TEST_EAX_NUM(num) \
	{                     \
		ASM_OP8(0xa9);    \
		ASM_LONG(num);    \
	} // test eax, 0x12
#define TEST_AL_NUM(num) \
	{                    \
		ASM_OP8(0xa8);   \
		ASM_BYTE(num);   \
	} // test al, 0x12
#define TEST_EDX_NUM(num)     \
	{                         \
		ASM_OP16(0xf7, 0xc2); \
		ASM_LONG(num);        \
	} // test edx, 0x12
#define TEST_DL_NUM(num)      \
	{                         \
		ASM_OP16(0xf6, 0xc2); \
		ASM_BYTE(num);        \
	} // test dl, 0x12

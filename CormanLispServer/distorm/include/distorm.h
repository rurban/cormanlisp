...
/* The Imm value is signed extended (E.G in 64 bit decoding mode, a 32 bit imm is usually sign extended into 64 bit
 * imm). */
...
#define OPERANDS_NO (4)
	...
	typedef struct
	{
		 ...
		 */
		uint8_t segment;
		/* Used by ops[n].type == O_MEM. Base global register index (might be R_NONE), scale size (2/4/8), ignored for 0
		 * or 1. */
		uint8_t base, scale;
		uint8_t dispSize;
...
/* The decoder will stop and return to the caller when any of the branch 'JMP', (near and far) instructions were
 * decoded. */
...
#define DF_STOP_ON_FLOW_CONTROL                                                                          \
	(DF_STOP_ON_CALL | DF_STOP_ON_RET | DF_STOP_ON_SYS | DF_STOP_ON_UNC_BRANCH | DF_STOP_ON_CND_BRANCH | \
 ...
 *         result - Array of type _DecodeInst which will be used by this function in order to return the disassembled
 * instructions. maxInstructions - The maximum number of entries in the result array that you pass to this function, so
 * it won't exceed its bound. usedInstructionsCount - Number of the instruction that successfully were disassembled and
 * written to the result array. Output: usedInstructionsCount will hold the number of entries used in the result array
 *         and the result array itself will be filled with the disassembled instructions.
 * Return: DECRES_SUCCESS on success (no more to disassemble), DECRES_INPUTERR on input error (null code buffer, invalid
 * decoding mode, etc...), DECRES_MEMORYERR when there are not enough entries to use in the result array, BUT YOU STILL
 * have to check for usedInstructionsCount! Side-Effects: Even if the return code is DECRES_MEMORYERR, there might STILL
 * be data in the array you passed, this function will try to use as much entries as possible! Notes:  1)The minimal
 * size of maxInstructions is 15. 2)You will have to synchronize the offset,code and length by yourself if you pass code
 * fragments and not a complete code block!
 */
 ...
 */
	...
	_DecodeResult distorm_decompose64(_CodeInfo* ci, _DInst result[], unsigned int maxInstructions,
...
#define distorm_decompose distorm_decompose64
	...
	_DecodeResult distorm_decode64(_OffsetType codeOffset, const unsigned char* code, int codeLen, _DecodeType dt,
...
#define distorm_format distorm_format64
	...
	_DecodeResult distorm_decompose32(_CodeInfo* ci, _DInst result[], unsigned int maxInstructions,
									  unsigned int* usedInstructionsCount);
#define distorm_decompose distorm_decompose32

#ifndef DISTORM_LIGHT
	/* If distorm-light is defined, we won't export these text-formatting functionality. */
	_DecodeResult distorm_decode32(_OffsetType codeOffset, const unsigned char* code, int codeLen, _DecodeType dt,
								   _DecodedInst result[], unsigned int maxInstructions,
								   unsigned int* usedInstructionsCount);

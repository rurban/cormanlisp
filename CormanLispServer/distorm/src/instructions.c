...
When you read a byte and have to decide if it's enough or you should read more bytes, 'till you get to the instruction
information. It's really fast because you POP the instruction info in top 3 iterates on the DB, because an instruction
can be formed from two bytes + 3 bits reg from the ModR/M byte. For a simple explanation, check this out:
...
How would such a thing look in memory, say we support 4 instructions with 3 bytes top (means 2 dynamically allocated
gates).
...
In Intel, you could go through 4 gates at top, because there are instructions which are built from 2 bytes and another
smaller list for the REG part, or newest SSE4 instructions which use 4 bytes for opcode. Therefore, Intel's first gate
is 256 long, and other gates are 256 (/72) or 8 long, yes, it costs pretty much a lot of memory for non-used defined
instructions, but I think that it still rocks.
*/
 ...
 * A case such that a REP prefix is being changed into an instruction byte and also an SSE instruction will not be found
 can't happen,
...
static _InstInfo* inst_lookup_prefixed(_InstNode in, _PrefixState* ps)
{
	...
	switch (ps->decodedPrefixes & (INST_PRE_OP_SIZE | INST_PRE_REPS))
	{
		...
		default:
			...
			if ((ps->decodedPrefixes & INST_PRE_REPS) == INST_PRE_REPS)
				return NULL;
...
static _InstInfo* inst_vex_lookup(_CodeInfo* ci, _PrefixState* ps)
{
	...
	if ((ps->decodedPrefixes & illegal) != 0)
		return NULL;
	 ...
	 */
	if ((instType == INT_INFO) || (instType == INT_INFOEX) || (instType == INT_LIST_DIVIDED))
		return NULL;
...
_InstInfo* inst_lookup(_CodeInfo* ci, _PrefixState* ps)
{
	...
	if (ps->decodedPrefixes & INST_PRE_VEX)
	{
		...
		if (ii != NULL)
		{
			...
			if ((((_InstInfoEx*)ii)->flagsEx & INST_FORCE_VEXL) && (~ps->vrex & PREFIX_EX_L))
				return NULL;
			/* If the instruction doesn't use VEX.vvvv it must be zero. */
			if ((((_InstInfoEx*)ii)->flagsEx & INST_VEX_V_UNUSED) && ps->vexV)
				return NULL;
		}
		return ii;
	}
	...
	if (tmpIndex0 == INST_WAIT_INDEX)
	{
		...
		ci->codeLen -= 1;
		if (ci->codeLen < 0)
			return NULL; /* Faster to return NULL, it will be detected as WAIT later anyway. */
	...
	if ((instType < INT_INFOS) && (!isWaitIncluded))
	{
		...
		switch (tmpIndex0)
		{
			case INST_ARPL_INDEX:
				 ...
				 * I have no choice but to check for ARPL/MOVSXD right here - "right about now, the funk soul brother,
			...
			case INST_NOP_INDEX: /* Nopnopnop */
				 ...
				 * 90 XCHG EAX, EAX is a true NOP (and not high dword of RAX = 0 although it should be a 32 bits
				 * operation). Note that if the REX.B is used, then the register is not RAX anymore but R8, which means
				 * it's not a NOP.
				 */
				if (rex & PREFIX_EX_W)
					ps->usedPrefixes |= INST_PRE_REX;
				if ((ci->dt != Decode64Bits) || (~rex & PREFIX_EX_B))
					return &II_NOP;
				break;
	...
	if (instType == INT_LIST_DIVIDED)
	{
		/* Checking for inst by REG bits is higher priority if it's found not to be divided instruction. */
		{
			...
			if (instType == INT_INFO)
				ii = &InstInfos[INST_NODE_INDEX(in2)];
			else if (instType == INT_INFOEX)
				ii = (_InstInfo*)&InstInfosEx[INST_NODE_INDEX(in2)];
			if ((ii != NULL) && (INST_INFO_FLAGS(ii) & INST_NOT_DIVIDED))
				return ii;
			/* ii is reset below. */
		}
		...
		if (instType < INT_INFOS)
		{
			...
			if ((~INST_INFO_FLAGS(ii) & INST_PRE_OP_SIZE) && (isWaitIncluded))
				return NULL;
			return ii;
		}
		/*
	...
	if (instType == INT_LIST_FULL)
	{
		...
		if (instType < INT_INFOS)
			return instType == INT_INFO ? &InstInfos[INST_NODE_INDEX(in)]
										: (_InstInfo*)&InstInfosEx[INST_NODE_INDEX(in)];
	...
	if (instType == INT_LIST_GROUP)
	{
		...
		if ((ii != NULL) && (ii->opcodeId == I_VMPTRLD) && (tmpIndex1 >= INST_DIVIDED_MODRM))
			return &II_RDRAND;
		return ii;
	}
	...
	if (instType == INT_LIST_DIVIDED)
	{
		...
		if (tmpIndex2 >= INST_DIVIDED_MODRM)
			return inst_get_info(in, tmpIndex2 - INST_DIVIDED_MODRM + 8);

		/* It might be that we got here without touching ii in the above if statements, then it becomes an invalid
		 * instruction prolly. */
		return ii;
	}
...
* So in order to tell the extract operands function which operands the 3DNow! instruction require, we need to set up
some
...
* which will be called only for 3DNow! instructions, distinguished by a flag, and it will read the last byte of the 3
bytes.
*

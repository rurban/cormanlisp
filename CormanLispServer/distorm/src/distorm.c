...
_DLLEXPORT_ _DecodeResult distorm_decompose64(_CodeInfo* ci, _DInst result[], unsigned int maxInstructions,
											  unsigned int* usedInstructionsCount)
#else
_DLLEXPORT_ _DecodeResult distorm_decompose32(_CodeInfo* ci, _DInst result[], unsigned int maxInstructions,
											  unsigned int* usedInstructionsCount)
#endif
{
	...
	*usedInstructionsCount = 0;
	...
	if ((ci == NULL) || (ci->codeLen < 0) ||
		((ci->dt != Decode16Bits) && (ci->dt != Decode32Bits) && (ci->dt != Decode64Bits)) || (ci->code == NULL) ||
		(result == NULL) ||
...
static void distorm_format_size(_WString* str, const _DInst* di, int opNum)
{
	...
	if (!isSizingRequired)
	{
		...
		switch (di->opcode)
		{
			...
			case I_SAR:
			case I_SHLD:
			case I_SHRD: isSizingRequired = 1; break;
...
#endif
{
	...
	if (ci->features & DF_MAXIMUM_ADDR32)
		addrMask = 0xffffffff;
	else if (ci->features & DF_MAXIMUM_ADDR16)
		addrMask = 0xffff;
	...
	{
		case FLAG_LOCK: strcpy_WSN(str, "LOCK "); break;
		case FLAG_REP:
			/* REP prefix for CMPS and SCAS is really a REPZ. */
			if ((di->opcode == I_CMPS) || (di->opcode == I_SCAS))
				strcpy_WSN(str, "REPZ ");
			else
				strcpy_WSN(str, "REP ");
			break;
		case FLAG_REPNZ: strcpy_WSN(str, "REPNZ "); break;
		default:
	...
	if ((META_GET_ISC(di->meta) == ISC_INTEGER) &&
		((di->opcode == I_MOVS) || (di->opcode == I_CMPS) || (di->opcode == I_STOS) || (di->opcode == I_LODS) ||
		 (di->opcode == I_SCAS)))
	{
		/*
	...
	for (i = 0; ((i < OPERANDS_NO) && (di->ops[i].type != O_NONE)); i++)
	{
		...
		{
			case O_REG: strcat_WS(str, (const _WString*)&_REGISTERS[di->ops[i].index]); break;
			case O_IMM:
				...
				if (di->ops[i].size == 64)
					str_code_hqw(str, (uint8_t*)&di->imm.qword);
				else
					str_code_hdw(str, di->imm.dword);
				break;
			case O_IMM1: str_code_hdw(str, di->imm.ex.i1); break;
			case O_IMM2: str_code_hdw(str, di->imm.ex.i2); break;
			case O_DISP:
				distorm_format_size(str, di, i);
			...
			case O_SMEM:
				 ...
				 * So we have to complete it ourselves, since the structure supplies only the segment that can be
				 * overridden. And make the rest of the String operations explicit.
				 */
	...
	if (di->flags & FLAG_HINT_TAKEN)
		strcat_WSN(str, " ;TAKEN");
	else if (di->flags & FLAG_HINT_NOT_TAKEN)
		strcat_WSN(str, " ;NOT TAKEN");
}

#ifdef SUPPORT_64BIT_OFFSET
_DLLEXPORT_ _DecodeResult distorm_decode64(_OffsetType codeOffset, const unsigned char* code, int codeLen,
										   _DecodeType dt, _DecodedInst result[], unsigned int maxInstructions,
										   unsigned int* usedInstructionsCount)
#else
_DLLEXPORT_ _DecodeResult distorm_decode32(_OffsetType codeOffset, const unsigned char* code, int codeLen,
										   _DecodeType dt, _DecodedInst result[], unsigned int maxInstructions,
										   unsigned int* usedInstructionsCount)
#endif
{
	...
	*usedInstructionsCount = 0;
	 ...
	 * Therefore, we will pass the result array(!) from the caller and the interal decoder will fill it in with
	 * _DInst's. Then we will copy each result to a temporary structure, and use it to reformat that specific result.
	 *
	...
	{
		if ((*usedInstructionsCount + i) >= maxInstructions)

...
uint32_t _REGISTERTORCLASS[] = /* Based on _RegisterType enumeration! */
	{RM_AX,	 RM_CX,	 RM_DX,	 RM_BX,	 RM_SP,	 RM_BP,	 RM_SI,	 RM_DI,	 RM_R8,	 RM_R9,	 RM_R10, RM_R11, RM_R12, RM_R13,
	 RM_R14, RM_R15, RM_AX,	 RM_CX,	 RM_DX,	 RM_BX,	 RM_SP,	 RM_BP,	 RM_SI,	 RM_DI,	 RM_R8,	 RM_R9,	 RM_R10, RM_R11,
	 RM_R12, RM_R13, RM_R14, RM_R15, RM_AX,	 RM_CX,	 RM_DX,	 RM_BX,	 RM_SP,	 RM_BP,	 RM_SI,	 RM_DI,	 RM_R8,	 RM_R9,
	 RM_R10, RM_R11, RM_R12, RM_R13, RM_R14, RM_R15, RM_AX,	 RM_CX,	 RM_DX,	 RM_BX,	 RM_AX,	 RM_CX,	 RM_DX,	 RM_BX,
	 RM_R8,	 RM_R9,	 RM_R10, RM_R11, RM_R12, RM_R13, RM_R14, RM_R15, RM_SP,	 RM_BP,	 RM_SI,	 RM_DI,	 0,		 0,
	 0,		 0,		 0,		 0,		 0,		 RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_FPU, RM_MMX,
	 RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_MMX, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE,
	 RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_SSE, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX,
	 RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_AVX, RM_CR,	 0,		 RM_CR,
	 RM_CR,	 RM_CR,	 0,		 0,		 0,		 RM_CR,	 RM_DR,	 RM_DR,	 RM_DR,	 RM_DR,	 0,		 0,		 RM_DR,	 RM_DR};
 ...
 * }
 */
static void operands_extract_sib(_DInst* di, _OperandNumberType opNum, _PrefixState* ps, _DecodeType effAdrSz,
								 unsigned int sib, unsigned int mod)
{
	...
	else if (mod != 0)
	{
		...
		if (effAdrSz == Decode64Bits)
			*pIndex = REGS64_BASE + 5 + ((vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0);
		else
			*pIndex = REGS32_BASE + 5 + ((vrex & PREFIX_EX_B) ? EX_GPR_BASE : 0);
	}
	else if (index == 4)
	{
	...
	if (index != 4)
	{ /* In 64 bits decoding mode, if index == R12, it's valid! */
		if (effAdrSz == Decode64Bits)
 ...
 */
static int operands_extract_modrm(_CodeInfo* ci, _DInst* di, _OpType type, _OperandNumberType opNum, _PrefixState* ps,
								  _DecodeType effOpSz, _DecodeType effAdrSz, int* lockableInstruction, unsigned int mod,
								  unsigned int rm, _iflags instFlags)
{
	...
	if (mod == 3)
	{
		 ...
		 * NOTE!! that we have to override the size of the register, since it was set earlier as Memory and not
		 * Register!
		 */
		op->type = O_REG;
		...
		switch (type)
		{
				...
				switch (effOpSz)
				{
					...
					case Decode64Bits:
						...
						if (type == OT_RFULL_M16)
							ps->usedPrefixes |= INST_PRE_REX;
						/* CALL NEAR/PUSH/POP defaults to 64 bits. --> INST_64BITS, REX isn't required, thus ignored
						 * anyways. */
			...
			case OT_R32_64_M16:
			/* FALL THROUGH, decode 32 or 64 bits register. */
			case OT_RM32_64: /* Take care specifically in MOVNTI/MOVD/CVT's instructions, making it _REG64 with REX or
	...
	if (effAdrSz == Decode16Bits)
	{
		...
		if ((mod == 0) && (rm == 6))
		{
			...
			op->type = O_DISP;
			di->dispSize = 16;
			if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int16_t)))
				return FALSE;
		}
		else
		{
			...
			if (mod == 1)
			{ /* 8 bits displacement + indirection */
				di->dispSize = 8;
				if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int8_t)))
					return FALSE;
			}
			else if (mod == 2)
			{ /* 16 bits displacement + indirection */
				di->dispSize = 16;
				if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int16_t)))
					return FALSE;
			}
	...
	{ /* Decode32Bits or Decode64Bits! */
		...
		if ((mod == 0) && (rm == 5))
		{
			...
			di->dispSize = 32;
			if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int32_t)))
				return FALSE;
		...
		else
		{
			if (rm == 4)
			{
				...
				/* Read SIB byte. */
				if (!read_stream_safe_uint(ci, &sib, sizeof(int8_t)))
					return FALSE;
			...
			{ /* If there is no BASE, read DISP32! */
				di->dispSize = 32;
				if (!read_stream_safe_sint(ci, (int64_t*)&di->disp, sizeof(int32_t)))
					return FALSE;
			}
 ...
 * Only by decoding the operands of an instruction which got a LOCK prefix, we could tell whether it may use the LOCK
 prefix.
 * According to Intel, LOCK prefix must precede some specific instructions AND in their memory destination operand form
 (which means first operand).
...
int operands_extract(_CodeInfo* ci, _DInst* di, _InstInfo* ii, _iflags instFlags, _OpType type,
					 _OperandNumberType opNum, unsigned int modrm, _PrefixState* ps, _DecodeType effOpSz,
					 _DecodeType effAdrSz, int* lockableInstruction)
{
	 ...
	 */
	mod = (modrm >> 6) &
		  3; /* Mode(register-indirection, disp8+reg+indirection, disp16+reg+indirection, general-purpose register) */
	...
	if (typeHandled)
	{
		...
		ret =
			operands_extract_modrm(ci, di, type, opNum, ps, effOpSz, effAdrSz, lockableInstruction, mod, rm, instFlags);
	...
	switch (type)
	{
		...
		case OT_R32_M8:
		case OT_R32_64_M8:
		case OT_REG32_64_M8: opSize = OPERAND_SIZE8; break;

		case OT_XMM16:
		case OT_R32_M16:
		case OT_R32_64_M16:
		case OT_REG32_64_M16: opSize = OPERAND_SIZE16; break;
		...
		case OT_MM64:
		case OT_XMM64: opSize = OPERAND_SIZE64; break;
	...
	switch (type)
	{
		case OT_IMM8:
			...
			if (!read_stream_safe_uint(ci, &di->imm.byte, sizeof(int8_t)))
				return FALSE;
			break;
		case OT_IMM_FULL: /* 16, 32 or 64, depends on prefixes. */
			if (effOpSz == Decode16Bits)
			{
				...
				case OT_IMM16: /* Force 16 bits imm. */
					...
					if (!read_stream_safe_uint(ci, &di->imm.word, sizeof(int16_t)))
						return FALSE;
					break;
					/*
					 ...
					 ((instFlags & (INST_64BITS | INST_PRE_REX)) == (INST_64BITS | INST_PRE_REX)))
			{
				...
				if (!read_stream_safe_uint(ci, &di->imm.qword, sizeof(int64_t)))
					return FALSE;
				break;
			}
			else
		...
		case OT_IMM32:
			...
			if (ci->dt == Decode64Bits)
			{
				...
				if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int32_t)))
					return FALSE;
			}
			else
			{
				op->size = 32;
				if (!read_stream_safe_uint(ci, &di->imm.dword, sizeof(int32_t)))
					return FALSE;
			}
			break;
		case OT_SEIMM8: /* Sign extended immediate. */
			...
			if ((instFlags & INST_PRE_OP_SIZE) && (ps->decodedPrefixes & INST_PRE_OP_SIZE))
			{
				...
				switch (ci->dt)
				{
					...
					case Decode32Bits:
					case Decode64Bits: op->size = 16; break;
				}
			}
			else
				op->size = 8;
			di->flags |= FLAG_IMM_SIGNED;
			if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int8_t)))
				return FALSE;
			break;
		case OT_IMM16_1:
			operands_set_ts(op, O_IMM1, 16);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i1, sizeof(int16_t)))
				return FALSE;
			break;
		case OT_IMM8_1:
			operands_set_ts(op, O_IMM1, 8);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i1, sizeof(int8_t)))
				return FALSE;
			break;
		case OT_IMM8_2:
			operands_set_ts(op, O_IMM2, 8);
			if (!read_stream_safe_uint(ci, &di->imm.ex.i2, sizeof(int8_t)))
				return FALSE;
			break;
		case OT_REG8:
			...
			break;
		case OT_REG16: operands_set_tsi(op, O_REG, 16, REGS16_BASE + reg); break;
		case OT_REG_FULL:
			switch (effOpSz)
			{
				case Decode16Bits:
		...
		case OT_REG32:
			...
			operands_set_tsi(op, O_REG, 32, REGS32_BASE + reg);
			break;
		case OT_REG32_64: /* Handle CVT's, MOVxX and MOVNTI instructions which could be extended to 64 bits registers
							 with REX. */
			if (vrex & PREFIX_EX_R)
			{
				ps->usedPrefixes |= INST_PRE_REX;
			...
			if (vrex & PREFIX_EX_W)
			{
				...
				operands_set_tsi(op, O_REG, 32, REGS32_BASE + reg);
			break;
		case OT_FREG32_64_RM: /* Force decoding mode. Used for MOV CR(n)/DR(n) which defaults to 64 bits operand size in
								 64 bits. */
			if (vrex & PREFIX_EX_B)
			{
				ps->usedPrefixes |= INST_PRE_REX;
			...
			if (ci->dt == Decode64Bits)
				operands_set_tsi(op, O_REG, 64, REGS64_BASE + rm);
			else
				operands_set_tsi(op, O_REG, 32, REGS32_BASE + rm);
			break;
		case OT_MM: /* MMX register */ operands_set_tsi(op, O_REG, 64, MMXREGS_BASE + reg); break;
		case OT_MM_RM: /* MMX register, this time from the RM field */
			operands_set_tsi(op, O_REG, 64, MMXREGS_BASE + rm);
			break;
		case OT_REGXMM0: /* Implicit XMM0 operand. */ reg = 0; vrex = 0;
		/* FALL THROUGH */
		...
		case OT_CREG:
			...
			else if ((ci->dt == Decode32Bits) && (ps->decodedPrefixes & INST_PRE_LOCK))
			{
				 ...
				 * if the lock prefix is set before MOV CR(n) it will become the 4th bit of the REG field like REX.R in
				 * 64 bits.
				 */
				reg += EX_GPR_BASE;
		...
		case OT_SREG: /* Works with REG16 only! */
			...
			if (reg <= SEG_REGS_MAX - 1)
				operands_set_tsi(op, O_REG, 16, SREGS_BASE + reg);
			else
				return FALSE;
			break;
		case OT_SEG:
			...
			break;
		case OT_ACC8: operands_set_tsi(op, O_REG, 8, R_AL); break;
		case OT_ACC16: operands_set_tsi(op, O_REG, 16, R_AX); break;
		case OT_ACC_FULL_NOT64: /* No REX.W support for IN/OUT. */ vrex &= ~PREFIX_EX_W;
		case OT_ACC_FULL:
			if (effOpSz == Decode16Bits)
			{
				ps->usedPrefixes |= INST_PRE_OP_SIZE;
			...
			break;
		case OT_PTR16_FULL:
			/* ptr16:full - full is size of operand size to read, therefore Operand Size Prefix affects this. So we need
			 * to handle it. */
			if (effOpSz == Decode16Bits)
			{
				ps->usedPrefixes |= INST_PRE_OP_SIZE;
			...
			{ /* Decode32Bits, for Decode64Bits this instruction is invalid. */
				ps->usedPrefixes |= INST_PRE_OP_SIZE;
		...
		case OT_RELC_FULL:
			...
			{ /* OT_RELC_FULL */
				...
				if (effOpSz == Decode16Bits)
				{
					...
					if (!read_stream_safe_sint(ci, &di->imm.sqword, sizeof(int16_t)))
						return FALSE;
				}
				else
				{ /* Decode32Bits or Decode64Bits = for now they are the same */
		...
		case OT_MOFFS_FULL:
			...
			if (effAdrSz == Decode16Bits)
			{
				...
				di->dispSize = 16;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int16_t)))
					return FALSE;
			}
			else if (effAdrSz == Decode32Bits)
			{
				...
				di->dispSize = 32;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int32_t)))
					return FALSE;
			}
			else
			{ /* Decode64Bits */
				di->dispSize = 64;
				if (!read_stream_safe_uint(ci, &di->disp, sizeof(int64_t)))
					return FALSE;
			}
			break;
		case OT_CONST1:
			...
			di->imm.byte = 1;
			break;
		case OT_REGCL: operands_set_tsi(op, O_REG, 8, R_CL); break;
		 ...
		 * If there's an operand size prefix, we will change the suffix letter of the mnemonic, which specifies the size
		 of operand to the required one.
		 * If there's a segment override prefix, we will output the segment and the used index register (EDI/ESI).
		 * If there's an address size prefix, we will output the (segment if needed and) the used and inverted index
		 register (DI/SI).
		 ...
		 * Therefore, we cannot change the mnemonic here when we encounter another prefix and its not the decoder's
		...
		case OT_REGECX: operands_set_tsi(op, O_REG, 32, R_ECX); break;
		case OT_REGI_EBXAL:
			/* XLAT BYTE [rBX + AL] */
			ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
		...
		case OT_REGI_EAX:
			...
			if (effAdrSz == Decode64Bits)
				operands_set_tsi(op, O_SMEM, 64, R_RAX);
			else if (effAdrSz == Decode32Bits)
			{
				ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
			...
			break;
		case OT_VXMM: operands_set_tsi(op, O_REG, 128, SSEREGS_BASE + vexV); break;
		case OT_XMM_IMM:
		...
		case OT_YMM:
			...
			operands_set_tsi(op, O_REG, 256, AVXREGS_BASE + reg);
			break;
		case OT_VYMM: operands_set_tsi(op, O_REG, 256, AVXREGS_BASE + vexV); break;
		case OT_VYXMM:
			if (ps->vrex & PREFIX_EX_L)

...
static _DecodeType decode_get_effective_op_size(_DecodeType dt, _iflags decodedPrefixes, unsigned int rex,
												_iflags instFlags)
{
	/*
	...
	if (dt == Decode64Bits)
	{
		...
		if (((instFlags & (INST_64BITS | INST_PRE_REX)) == INST_64BITS) ||
			...
			((decodedPrefixes & INST_PRE_REX) && (rex & PREFIX_EX_W)))
			dt = Decode64Bits;
		else
			dt = Decode32Bits; /* Default. */
	}
	return dt;
}

/* A helper macro to convert from diStorm's CPU flags to EFLAGS. */
#define CONVERT_FLAGS_TO_EFLAGS(dst, src, field)                                                   \
	dst->field = ((src->field & D_COMPACT_SAME_FLAGS) | ((src->field & D_COMPACT_IF) ? D_IF : 0) | \
				  ((src->field & D_COMPACT_DF) ? D_DF : 0) | ((src->field & D_COMPACT_OF) ? D_OF : 0));
...
static _DecodeResult decode_inst(_CodeInfo* ci, _PrefixState* ps, _DInst* di)
{
	 ...
	 */
	if ((ps->prefixExtType == PET_REX) && (ps->decodedPrefixes & INST_PRE_OP_SIZE) && (!ps->isOpSizeMandatory) &&
		(vrex & PREFIX_EX_W))
	{
	...
	if ((ci->dt == Decode64Bits) && (instFlags & INST_INVALID_64BITS))
		goto _Undecodable;

	/* If it's only a 64 bits instruction drop it in other decoding modes. */
	if ((ci->dt != Decode64Bits) && (instFlags & INST_64BITS_FETCH))
		goto _Undecodable;
	...
	if (instFlags & INST_MODRM_REQUIRED)
	{
		...
		if ((instFlags & INST_FORCE_REG0) && (((modrm >> 3) & 7) != 0))
			goto _Undecodable;
		/* Some instructions enforce that mod=11, so validate that. */
		if ((instFlags & INST_MODRR_REQUIRED) && (modrm < INST_DIVIDED_MODRM))
			goto _Undecodable;
	}
	...
	for (;;)
	{
		if (isi->d != OT_NONE)
		{
			if (!operands_extract(ci, di, ii, instFlags, (_OpType)isi->d, ONT_1, modrm, ps, effOpSz, effAdrSz,
								  &lockable))
				goto _Undecodable;
		}
		else
		...
		if (isi->s != OT_NONE)
		{
			if (!operands_extract(ci, di, ii, instFlags, (_OpType)isi->s, ONT_2, modrm, ps, effOpSz, effAdrSz, NULL))
				goto _Undecodable;
		}
		else
			break;
		...
		{
			if (!operands_extract(ci, di, ii, instFlags, (_OpType)((_InstInfoEx*)ii)->op3, ONT_3, modrm, ps, effOpSz,
								  effAdrSz, NULL))
				goto _Undecodable;
		}
		else
		...
		{
			if (!operands_extract(ci, di, ii, instFlags, (_OpType)((_InstInfoEx*)ii)->op4, ONT_4, modrm, ps, effOpSz,
								  effAdrSz, NULL))
				goto _Undecodable;
		}
		break;
	...
	/* If it were a 3DNow! instruction, we will have to find the instruction itself now that we got its operands
	 * extracted. */
	...
	/*
	 * If we reached here the instruction was fully decoded, we located the instruction in the DB and extracted
	 * operands. Use the correct mnemonic according to the DT. If we are in 32 bits decoding mode it doesn't necessarily
	 * mean we will choose mnemonic2, alas, it means that if there is a mnemonic2, it will be used.
	 */
	...
	{ /* Decode64Bits, note that some instructions might be decoded in Decode32Bits above. */
		...
		if (instFlags & (INST_USE_EXMNEMONIC | INST_USE_EXMNEMONIC2))
		{
			 ...
			 */
			if ((instFlags & INST_MNEMONIC_MODRM_BASED) && (modrm >= INST_DIVIDED_MODRM))
				goto _Undecodable;
 ...
 * decode_internal
 *
 * supportOldIntr - Since now we work with new structure instead of the old _DecodedInst, we are still interested in
 * backward compatibility. So although, the array is now of type _DInst, we want to read it in jumps of the old array
 * element's size. This is in order to save memory allocation for conversion between the new and the old structures. It
 * really means we can do the conversion in-place now.
 */
_DecodeResult decode_internal(_CodeInfo* _ci, int supportOldIntr, _DInst result[], unsigned int maxResultCount,
							  unsigned int* usedInstructionsCount)
{
	 ...
	 * We need this variable in addition to codeOffset, because prefixes might change the real offset an instruction
	 * begins at. So we keep track of both.
	 */
	...
	*usedInstructionsCount = 0;
	...
	while (codeLen > 0)
	{
		...
		if (prefixes_is_valid(*code, ci.dt))
		{
			...
			if ((codeLen == 0) || (prefixSize == INST_MAXIMUM_SIZE))
			{
				if (~_ci->features & DF_RETURN_FC_ONLY)
				{
					...
					if (nextPos + (ps.last - code) > maxResultCount)
		 ...
		 * so prefixSize includes that, and the returned size in pdi is simply the size of the real(=without prefixes)
		 instruction.
		 */
		...
		*usedInstructionsCount = nextPos;
		...
		if ((decodeResult == DECRES_SUCCESS) && (features & DF_STOP_ON_FLOW_CONTROL))
		{
			if (((features & DF_STOP_ON_CALL) && (mfc == FC_CALL)) ||
				((features & DF_STOP_ON_RET) && (mfc == FC_RET)) || ((features & DF_STOP_ON_SYS) && (mfc == FC_SYS)) ||
				((features & DF_STOP_ON_UNC_BRANCH) && (mfc == FC_UNC_BRANCH)) ||
				((features & DF_STOP_ON_CND_BRANCH) && (mfc == FC_CND_BRANCH)) ||
				((features & DF_STOP_ON_INT) && (mfc == FC_INT)) || ((features & DF_STOP_ON_CMOV) && (mfc == FC_CMOV)))
				return DECRES_SUCCESS;
		}
	}

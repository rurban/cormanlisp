...
inline char UpperAlpha(char c)
{
	return (char)(islower(c) ? toupper(c) : c);
}

inline long LowerAlpha(long c)
{
...
static void outputInteger(LispObj num, LispObj os, int base)
{
...
static void outputFloat(LispObj num, LispObj os)
{
	double d = 0.0;

	if (isDoubleFloat(num))
		d = doubleFloat(num);
	else if (isSingleFloat(num))
		d = singleFloat(num);
	else if (isShortFloat(num))
		d = shortFloat(num);
	static char str[80];
...
static void outputCharacter(LispObj n, LispObj os)
{
	outputChar(n, os);
}

void outputSymbol(LispObj n, LispObj os)
{
	LispObj name = 0;
	name = symbolName(n);
...
static void outputText(LispObj n, LispObj os)
{
	outputChars(n, integer(vectorLength(n)), os);
}

static void outputFunction(LispObj n, LispObj os)
{
	outputUnreadableObject(n, stringNode("FUNCTION"), os);
}

static void outputStream(LispObj n, LispObj os)
{
	outputUnreadableObject(n, stringNode("STREAM"), os);
}

static void outputList(LispObj s, LispObj os)
{
	if (s == UNINITIALIZED)
	{
...
void outputChars(LispObj chars, long start, long length, LispObj os)
{
	long i = 0;
...
LispObj consoleOverflow(LispObj s)
{
	...
	else
	{
		...
		numBytes =
			expandLineFeedsIntoBuffer(charArrayStart(streamOutputBuffer(s)), integer(bytesToWrite), CharOverflowBuffer);
		if (CormanLispServer)
		{
...
LispObj fileOverflow(LispObj s)
{
	...
	if (streamBinary(s) != NIL) // if binary stream
	{
		...
		numBytes = integer(bytesToWrite);
		ret = WriteFile((void*)lispIntegerToLong(streamHandle(s)), // handle of file to write
	...
	if (!ret)
		Error("Could not write to file ~A, error code = ~A", s, createLispInteger(GetLastError()));

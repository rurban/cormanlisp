...
void initializeGarbageCollector()
{
	...
	LispHeap2.alloc(EphemeralHeap1Size + EphemeralHeap2Size + LispHeapSize, LispHeapSize, 2, LispHeapReserveDefault);
...
{
	return EphemeralHeap1.inHeap((byte*)addr) || EphemeralHeap2.inHeap((byte*)addr) || LispHeap1.inHeap((byte*)addr);
}
...
static void gcVerifyHeap(LispHeap* h, const char* name, const char* phase)
{
	...
	while (p < h->current)
	{
		if (gettag(p->car) == UvectorLengthTag)
		{
			...
			{
				fprintf(stderr, "[GCVERIFY %s] %s: bad uvector header at %p hdr=0x%08lx cells=%ld (heap [%p..%p))\n",
						phase, name, (void*)p, (unsigned long)p->car, cells, (void*)h->start, (void*)h->current);
				gcVerifyErrors++;

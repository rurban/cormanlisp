...
static PageTableEntry* allocPageTable()
{
	...
	mem = (byte*)VirtualAlloc(NULL, (maxPage + 1) * sizeof(PageTableEntry), MEM_COMMIT, PAGE_EXECUTE_READWRITE);
...
#define isHeapPointer(n) \
	(isHeapBlock(n) && ((n) >= (LispObj)(GCFromSpace->start)) && ((n) < (LispObj)(GCFromSpace->end)))

#define pointsIntoHeap(n) (((n) >= (LispObj)(GCFromSpace->start)) && ((n) < (LispObj)(GCFromSpace->end)))
...
static int growLispHeaps(long numCells)
{
	...
	if (LispHeap1.grow(bytesToGrow) || LispHeap2.grow(bytesToGrow))
	{
		return -1;
	}
...
LispHeap::LispHeap()
	: start(0), end(0), current(0), overflow(0), mem(0), generation(0), sizeMem(0), firstPage(0), numPages(0),
	  reserve(0)
{
}

void LispHeap::alloc(unsigned long size, unsigned long overflowVal, unsigned long gen, unsigned long reserveRequested)
{
...
void LispHeap::writeProtectAllPages()
{
	if (HardwareAssist)
	{
		...
		BOOL ret = 0;
		DWORD err = 0;
		ret = VirtualProtect(page_address(firstPage), PAGE_SIZE * numPages, PAGE_EXECUTE_READ, &oldProtect);
		if (!ret)
		{
			err = GetLastError();
		}
...
void LispHeap::unWriteProtectAllPages()
{
	...
	BOOL ret = 0;
	DWORD err = 0;
	ret = VirtualProtect(page_address(firstPage), PAGE_SIZE * numPages, PAGE_EXECUTE_READWRITE, &oldProtect);
	if (!ret)
	{
		err = GetLastError();
	}
...
int LispHeap::decommitAllPages()
{
	...
	if (firstUncommittedPage > firstPage)
		ret = VirtualFree((void*)page_address(firstPage), PAGE_SIZE * (firstUncommittedPage - firstPage), MEM_DECOMMIT);
	firstUncommittedPage = firstPage;
	if (!ret)
	{
		err = GetLastError();
...
int LispHeap::commitPage(byte* addr, int readonly)
{
	...
	ret = (void*)VirtualAlloc((byte*)page_address(firstUncommittedPage),
							  (newpage - firstUncommittedPage + 1) * PAGE_SIZE, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
...
extern "C" void __enter_gc_critical_section()
{
	LISP_TO_FOREIGN()
...
//	__asm		pop edi
	...
	void initializeGarbageCollector()
	{
		...
		LispHeap2.decommitAllPages();
#ifdef _DEBUG
		fprintf(stderr, "[initGC] EphemeralHeap1: %p..%p (%ld MB)\n", (void*)EphemeralHeap1.start,
				(void*)EphemeralHeap1.end, (long)(((char*)EphemeralHeap1.end - (char*)EphemeralHeap1.start) >> 20));
	...
	void handleUntaggedValues(LispObj uvec, LispObj index)
	{
		...
		if (startPage != page && offset != 0 &&
			((isSimpleVector(uvec) && !isSimpleGenericVector(uvec)) || isCompiledCode(uvec)))
		{
	...
	{
		return EphemeralHeap1.inHeap((byte*)addr) || EphemeralHeap2.inHeap((byte*)addr) ||
...
#ifdef _DEBUG
	// Strict heap consistency checker: walks every block of the live heaps,
	// validating uvector headers and (for cons cells) that heap pointers land
	// on plausible blocks. A ForwardTag header behind a live reference means
	// the GC moved the target but never updated this reference.
	static long gcVerifyErrors = 0;

	static int gcVerifyInLiveRange(LispObj addr, LispHeap * h)
	{
		return addr >= (LispObj)h->start && addr < (LispObj)h->current;
	}

	static int gcVerifyInReservedRange(LispObj addr, LispHeap * h)
	{
		return addr >= (LispObj)h->start && addr < (LispObj)h->end;
	}

	static void gcVerifySlotValue(LispObj v, LispObj * slot, const char* heapName, const char* phase)
	{
		if (!isHeapBlock(v))
			return;
		LispObj addr = stripTag(v);
		if (!addr)
			return; // UNINITIALIZED / SYMBOL_UNBOUND
		LispHeap* target = 0;
		const char* targetName = 0;
		if (gcVerifyInReservedRange(addr, &EphemeralHeap1))
			target = &EphemeralHeap1, targetName = "E1";
		else if (gcVerifyInReservedRange(addr, &EphemeralHeap2))
			target = &EphemeralHeap2, targetName = "E2";
		else if (gcVerifyInReservedRange(addr, &LispHeap1))
			target = &LispHeap1, targetName = "L1";
		else
			return; // static/global or foreign — not our concern
		if (!gcVerifyInLiveRange(addr, target))
		{
			fprintf(stderr, "[GCVERIFY %s] %s slot %p holds %p -> dead space of %s [%p..%p) *target=0x%08lx\n", phase,
					heapName, (void*)slot, (void*)v, targetName, (void*)target->start, (void*)target->current,
					*(unsigned long*)addr);
			gcVerifyErrors++;
			return;
		}
		LispObj hdr = *(LispObj*)addr;
		if (isForwardingPtr(hdr))
		{
			fprintf(stderr, "[GCVERIFY %s] %s slot %p holds %p -> forwarding ptr 0x%08lx (missed update)\n", phase,
					heapName, (void*)slot, (void*)v, (unsigned long)hdr);
			gcVerifyErrors++;
			return;
		}
		if (gettag(v) == UvectorTag && gettag(hdr) != UvectorLengthTag)
		{
			fprintf(stderr, "[GCVERIFY %s] %s slot %p holds uvector ptr %p but target hdr=0x%08lx\n", phase, heapName,
					(void*)slot, (void*)v, (unsigned long)hdr);
			gcVerifyErrors++;
		}
	}

	static void gcVerifyHeap(LispHeap * h, const char* name, const char* phase)
	{
		Node* p = h->start;
		while (p < h->current)
		{
			...
			{
				long cells = ((long)(p->car)) >> 8;
				if (cells <= 0 || p + cells > h->current)
				{
					fprintf(stderr,
							"[GCVERIFY %s] %s: bad uvector header at %p hdr=0x%08lx cells=%ld (heap [%p..%p))\n", phase,
							name, (void*)p, (unsigned long)p->car, cells, (void*)h->start, (void*)h->current);
					gcVerifyErrors++;
					return; // heap walk is broken from here on
				}
				p += cells;
			}
			else
			{
				gcVerifySlotValue(p->car, &p->car, name, phase);
				gcVerifySlotValue(p->cdr, &p->cdr, name, phase);
				p++;
			}
		}
	}

	static void gcVerifyAllHeaps(const char* phase)
	{
		gcVerifyErrors = 0;
		gcVerifyHeap(&EphemeralHeap1, "E1", phase);
		gcVerifyHeap(&EphemeralHeap2, "E2", phase);
		gcVerifyHeap(&LispHeap1, "L1", phase);
		if (gcVerifyErrors)
		{
			fprintf(stderr, "[GCVERIFY %s] GC #%lu: %ld error(s) — aborting\n", phase, garbageCollectionID,
					gcVerifyErrors);
			fflush(stderr);
			abort();
		}
		...
		__try
		{
			__try
			{
				...
				fprintf(stderr, "[GC] real GC #%lu level=%ld EphemeralHeap1: used=%ld/%ld MB\n", garbageCollectionID,
						level, (long)((char*)EphemeralHeap1.current - (char*)EphemeralHeap1.start) >> 20,
						EphemeralHeap1.sizeMem >> 20);
				fflush(stderr);
#endif
			...
			asm volatile("mov %%ebp, %0" : "=r"(gBasePointer) : : "memory");
				...
				__try
				{
					...
					// Note: only pre-existing E2 data [start..mark1) is scanned here; blocks
					// promoted during this collection are handled by copyReferencedBlocks().
					// L1 references into E2 do not need updating at level 0 (E2 doesn't move);
					// promoting E2 objects into E1 here would be fatal — E1 is reset below.
					...
					if (EphemeralHeap2.current > EphemeralHeap2.overflow || level > 0)
					{
						...
						if (((EphemeralHeap2.current - EphemeralHeap2.start) + LispHeap1.current) >= LispHeap1.end)
						{
	...
	static void checkQVRoots(LispObj * qv)
	{
		long i = 0;
	...
	static void promoteThreadExecutingFunctions(ThreadRecord * th)
	{
		...
		while (index > 0 && ((unsigned long)esp) > qv[STACK_MARKERS_Index + ((index - 1) * 2)])
			index--;
		...
		while (basePointer < start)
		{
			...
			while (index > 0 && ((unsigned long)basePointer) >= qv[STACK_MARKERS_Index + ((index - 1) * 2)])
				index--;
			if (!EVEN(index)) // if not a lisp segment
			{
				// we are into a non-lisp segment of the stack.
	...
	unsigned long looksLikeEBP(unsigned long addr)
	{
		unsigned long ret = (!(*(LispObj*)addr & 0x3)) // must be multiple of 4
							&& (*(LispObj*)addr > addr) && (*(LispObj*)addr < (addr + 4096));
		//	assert(!ret || looksLikeEBP(*(LispObj*)addr)); // this would recurse indefinitely
		if (ret)
		{
			ret = ((!(*(LispObj*)(*(LispObj*)addr) & 0x3)) && (*(LispObj*)(*(LispObj*)addr) > (*(LispObj*)addr)) &&
	...
	static void checkThreadStackRoots(ThreadRecord * th, LispHeap* /*fromSpace*/, LispHeap * toSpace)
	{
		static CONTEXT lispContext;
		LispObj* start = th->stackStart;
		LispObj* x = 0;
		...
		if (index > 0)
		{
			...
			for (x = start; x >= end; x--)
			{
				...
				else if (EVEN(i) && isHeapPointer(*x))
				{
...
#ifdef _MSC_VER
			__asm mov regs[0], eax __asm mov regs[4], ebx __asm mov regs[8], ecx __asm mov regs[12],
				edx __asm mov regs[16], esi __asm mov regs[20],
				edi
#else
	...
	static Node* checkObject(Node * p)
	{
		...
		if (gettag(p->car) == UvectorLengthTag)
		{
			...
			switch (type)
			{
				...
				case SimpleBitVectorType: break;
				...
				case CompiledCodeType:
					...
					if (GCCheckCode)
					{
						if (isHeapPointer(u[COMPILED_CODE_REFERENCES]) || isFixnum(u[COMPILED_CODE_REFERENCES]))
						{
							...
							else if (isFixnum(u[COMPILED_CODE_REFERENCES]) && u[COMPILED_CODE_REFERENCES] > 0)
							{
				...
				case StructureType:
					...
					{
						// mark hash table
						handleMemoryAccessException(
							(byte*)&(u[2])); // need to ensure page is committed and not write-protected
	...
	static Node* scanObjectForRoots(Node * p, long* updated)
	{
		LispObj uvec = 0;
		LispObj* u = 0;
		long type = 0;
	...
	static BOOL scanPageForRoots(unsigned long page_id)
	{
		...
		if (offset < CELLS_PER_PAGE)
		{
			...
			assert(GCToSpace->generation != 0); // never promote INTO the allocation heap
		}
		return updated;
	}
	...
	void copyReferencedBlocks(LispHeap * fromSpace, LispHeap * toSpace, Node * start)
	{
		GCFromSpace = fromSpace;
		GCToSpace = toSpace;
		GCScanSpace = toSpace;
	...
	static void checkHeapRoots(LispHeap * fromSpace, LispHeap * toSpace, LispHeap * checkHeap, Node * start, Node * end)
	{
		...
		else if (fromSpace->generation == 1)
		{
			for (i = start_page_id; i < end_page_id; i++)
			{
				flag_bits = PageFlags(i);
	...
	static void crossesPageBoundary(LispObj uvec)
	{
		if (isSimpleVector(uvec) && !isSimpleGenericVector(uvec))
		{
			handleUntaggedValues(uvec, wrapInteger(2));
		}
		else if (isKFunction(uvec))
		{
			handleUntaggedValues(uvec, wrapInteger(2));
		}
		else if (isDoubleFloat(uvec))
		{
			handleUntaggedValues(uvec, wrapInteger(2));
		}
		else if (isCompiledCode(uvec))
		{
			handleUntaggedValues(uvec, wrapInteger(4));
		}
		else if (isBignum(uvec))
		{
			handleUntaggedValues(uvec, wrapInteger(2));
		}
	}
		...
		switch (type)
		{
			...
			case ForeignStackType:
				if (length != 2)
					return 0;
				break;
			case ForeignStackEndType: break;
		}
		return 1;
	}

	void promoteBlock(LispObj * ptr, LispHeap * toSpace)
	{
	...
	{
		return (p >= (unsigned long)LispHeap2.start && p < (unsigned long)LispHeap2.end);
	}

	static int mayBeForwardingPointer(LispObj p)
	{
		return isForwardingPtr(p) && ((p >= (unsigned long)LispHeap2.start && p < (unsigned long)LispHeap2.end) ||
									  (p >= (unsigned long)LispHeap1.start && p < (unsigned long)LispHeap1.end));
	}
	...
	CHeapBlocks::CHeapBlocks() : m_buf(0), m_bufLength(0), m_count(0), m_length(0)
	{
		cblocks = this;
	...
	void CHeapBlocks::add(void* paddr, long size, long type)
	{
		...
		*(long*)(m_buf + m_length + sizeof(long) + sizeof(void*)) = size;
		memcpy(m_buf + m_length + sizeof(long) + sizeof(void*) + sizeof(long), *(unsigned char**)paddr, size);
	...
	const char* LispImageCopyright =
		"Corman Lisp Copyright (c) Corman Technologies. See LICENSE.txt for license information.";
	...
	int fileIsEXE(LispObj path)
	{
		...
		if (((c1 == 'E' || c1 == 'e') && (c2 == 'X' || c2 == 'x') && (c3 == 'E' || c3 == 'e')) ||
			((c1 == 'D' || c1 == 'd') && (c2 == 'L' || c2 == 'l') && (c3 == 'L' || c3 == 'l')))
		{
...
#define PaddedSize(n) ((n) % PAGE_SIZE ? (n) + (PAGE_SIZE - ((n) % PAGE_SIZE)) : (n))
	...
	static int finishEXE(FILE * exe, const char* sectionName)
	{
		...
		LispSectionHeader.VirtualAddress =
			PrevSectionHeader.VirtualAddress + PaddedSize(PrevSectionHeader.Misc.VirtualSize);
	...
	extern "C" int compress(byte * dest, unsigned long* destLen, byte* source, unsigned long sourceLen);
	extern "C" int uncompress(byte * dest, unsigned long* destLen, byte* source, unsigned long sourceLen);
	...
	void writeHeap(FILE * writeHeap_os)
	{
		...
		header.uncompressedSize = staticSize + sizeof(heapAddress) + heapSize + pageTableSize + CHeapBlocksSize;
		...
		{
			// compress the buffer
			compret = compress(compressionDestBuffer, &destLen, compressionSrcBuffer, header.uncompressedSize);
			if (compret != 0)
			{
				freeCompressionBuffer(compressionSrcBuffer);
			...
			__try
			{
				...
				if (header.compressionType == 1) // gz compression
				{
					...
					if (length != header.compressedSize)
						Error("Invalid lisp image file");
					compret = uncompress(compressionDestBuffer, &destLen, compressionSrcBuffer, header.compressedSize);
				...
				memcpy((char*)(PageTable + LispHeap1.firstPage), compressionDestBuffer + index, header.pageMapSize);
				...
				while (p < c_blocks + header.foreignBlocksSize)
				{
					...
					*(long*)(((char*)paddr) + 8) = wrapInteger(type);
					...
					else if (type == FOREIGN_HEAP_TYPE_CRITICAL_SECTION)
					{
...
#define ADJUST(place)                                 \
	...
	static void offsetUvector(LispObj obj, long offset, long length)
	{
		...
		switch (type)
		{
			...
			case KFunctionType: ADJUST(UVECTOR(obj)[FUNCTION_ENVIRONMENT]); break;
			...
			case ForeignType: break;

			case CompiledCodeType: offsetCode(obj, offset); break;
			...
			case BignumType:
			case ForeignHeapType: break;

			case WeakPointerType: ADJUST(UVECTOR(obj)[WEAK_PTR]); break;
	...
	static void offsetCode(LispObj obj, long offset)
	{
		...
		if (isHeapPointer(UVECTOR(obj)[COMPILED_CODE_REFERENCES]) || isFixnum(UVECTOR(obj)[COMPILED_CODE_REFERENCES]))
		{
			// update the code references
			...
			else if (isFixnum(UVECTOR(obj)[COMPILED_CODE_REFERENCES]) && UVECTOR(obj)[COMPILED_CODE_REFERENCES] > 0)
			{
				// the references are stored following the code
				// u[COMPILED_CODE_REFERENCES] = offset from start of code to reference header
				// Reference tags are 16 bit non-tagged integers, each representing an offset
	...
	void dumpHeapToFile(LispObj /*path*/)
	{
#if 0
    Node* p = 0;
    long numUvectors = 0;
...
static LispObj findFunctionCallback(LispObj p)
{
	...
	{
		FINALIZATION_REGISTRY = cons(cons(obj, func), FINALIZATION_REGISTRY);
	}
	...
	LispFunction(verifyHeapBlock)
	{
		...
		if (size > 0x10000)
		{
			setSymbolValue(INVALID_OBJECT_COUNT, symbolValue(INVALID_OBJECT_COUNT) + wrapInteger(1));
			found = 1;
		}
		LISP_FUNC_RETURN(NIL);
	}
	...
	static BYTE* MapFile(const char* path, DWORD* length)
	{
		HANDLE hfile = CreateFile(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING,
		...
		*length = GetFileSize(hfile, 0);
		...
		HANDLE hfilemap = CreateFileMapping(hfile, 0, PAGE_READWRITE, 0, 0, 0);

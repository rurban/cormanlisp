...
static void outputUvector(FILE* file, LispObj uvector)
{
	...
	switch (type)
	{
		...
		case KFunctionType:
			...
			break;
		case StructureType: fprintf(file, "{Structure}"); break;
		case ArrayType: fprintf(file, "{Array}"); break;
		case SymbolType:
			fprintf(file, "{Symbol: ");
			writeSymbolName(file, uvector);
			fprintf(file, "}");
			break;
		case StreamType: fprintf(file, "{Stream}"); break;
		case DoubleFloatType: fprintf(file, "{Double Float}"); break;
		case PackageType: fprintf(file, "{Package}"); break;
		case HashtableType: fprintf(file, "{Hashtable}"); break;
		case ForeignType: fprintf(file, "{Foreign Object}"); break;
		case CompiledCodeType: fprintf(file, "{Compiled Code}"); break;
		case ReadTableType: fprintf(file, "{Readtable}"); break;
		case ComplexType: fprintf(file, "{Complex}"); break;
		case RatioType: fprintf(file, "{Ratio}"); break;
		case BignumType: fprintf(file, "{Bignum}"); break;
		case ForeignHeapType: fprintf(file, "{Foreign Heap Object}"); break;
		case WeakPointerType: fprintf(file, "{Weak Pointer}"); break;
		case SimpleVectorType: fprintf(file, "{Simple Vector}"); break;
		case SimpleCharVectorType:
			...
			fprintf(file, "}");
			break;
		case SimpleByteVectorType: fprintf(file, "{Simple Byte Vector}"); break;
		case SimpleShortVectorType: fprintf(file, "{Simple Short Vector}"); break;
		case SimpleDoubleFloatVectorType: fprintf(file, "{Simple Double Float Vector}"); break;
		case SimpleBitVectorType: fprintf(file, "{Simple Bit Vector}"); break;
		case SimpleSingleFloatVectorType: fprintf(file, "{Simple Single Float Vector}"); break;
		case SingleFloatType: fprintf(file, "{Single Float}"); break;
		case CLOSInstanceType: fprintf(file, "{CLOS Instance}"); break;
		case ForeignStackType: fprintf(file, "{Foreign Stack Marker}"); break;
		case ForeignStackEndType: fprintf(file, "{Foreign Stack End Marker}"); break;
	}
}
...
static void outputReferencedObject(FILE* file, LispObj obj, int indent)
{
	...
	if (isFixnum(obj))
	{
		if (obj != 0 && (void*)obj <= stackStart && (void*)obj >= stackEnd)
			fprintf(file, "{Warning: Integer %lx or possible stack reference: %08lx}", integer(obj),
					obj); // may indicate a reference into the stack
		else
	...
	else if (isCons(obj))
	{
		...
		{
			fprintf(file, "{Cons (in %s): CAR = %08lx, CDR = %08lx}", getHeapName(obj), CAR(obj), CDR(obj));
		}
	}
...
void writeThreadSummary(FILE* file, ThreadRecord* th)
{
	...
	for (x = start; x >= end; x--)
	{
		...
		else if (stack[x - end].returnAddress != 0)
		{
			...
			while (index > 0)
			{
				...
				{
					frameName = lookupFunctionName(stack[index].value, &funcBase, FunctionNameBuffer,
		...
		if (EVEN(i))
		{
			if (inAnyLispHeap(*x) && *(unsigned long*)x == JumpBufferMarker)
			{
				...
				if (*(unsigned char*)x == ForeignStackEndMarker &&
					...
					&& (x - (2 * (*x >> 8))) > end && *(unsigned char*)(x - (2 * (*x >> 8))) == ForeignStackStartMarker)
				{
					// foreign stack block
					x -= ((2 * (*x >> 8)) - 1);
				}
	...
	if (!frameName)
		frameName = "<Unknown>";
	fprintf(file, "Thread ID %08lx  Executing function: %s  Execution context: %s", th->threadID, frameName,
			executionContext);
}
	...
	for (x = start; x >= end; x--)
	{
		...
		if (EVEN(i))
		{
			...
			if (inAnyLispHeap(*x) && *(unsigned long*)x == JumpBufferMarker)
			{
				...
				{
					// foreign stack block
					fprintf(file, "{Foreign Stack Block: Start = %08lx, Length = %08lx bytes}", *x,
							(2 * (*x >> 8)) * 4);
					x -= ((2 * (*x >> 8)) - 1);
				}
				else
...
void dumpUvector(FILE* file, LispObj u)
{
	...
	fprintf(dumpFile, "  %08lx: UVECTOR %08lx bytes     %s", stripTag(u), blockLength, UvectorTypeNames[type]);
	if (type == SymbolType)
	{
		fprintf(dumpFile, ":             ");
	...
	__try
	{
		__try
		{
			...
			if (err != 0)
			{
				...
				MessageBox(wnd,
						   "A system report could not be written because an error was encountered on\n"

...
ThreadRecord::ThreadRecord()
	: event(FALSE, TRUE), next(0), QV_rec(0), thread(0), threadID(0), stackStart(0), started(0), type(-1),
	  image_loads_count(getImageLoadsCount())
{
}

ThreadRecord::~ThreadRecord() {}

LispThreadQueue::LispThreadQueue() : list(0)
{
...
void LispThreadQueue::insert(ThreadRecord* rec)
{
...
void LispThreadQueue::suspendAllOtherThreads()
{
	DWORD ret = 0;
	int wait = 1;
	TQCriticalSection.Enter();
...
void LispThreadQueue::ensureSafeState(ThreadRecord* tr)
{
	...
	int wait = 1;
	while ((/*inAnyLispHeap(ip) &&*/ DirectionFlagSet(flags)) || inSysGlobalsHeap(ip) ||
		   (ip && ((*(unsigned char*)ip == 0x55) // push ebp

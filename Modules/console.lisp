...
(defun setup-console (&optional (input-buffer-length 2048) (output-buffer-length 2048))
    (win:AllocConsole)
    (setf (uref *terminal-io* cl::stream-underflow-func-offset) #'dll-terminal-io-underflow)
    (setf (uref *terminal-io* cl::stream-overflow-func-offset) #'dll-terminal-io-overflow)
    (setf (uref *terminal-io* cl::stream-input-buffer-offset)
        (make-array input-buffer-length :element-type 'character))
    (setf (uref *terminal-io* cl::stream-input-buffer-length-offset) input-buffer-length)

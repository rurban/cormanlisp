...
bool ExtractResource(const HMODULE hModule, const LPCTSTR lpName, const char* fname, const unsigned long magic)
{
	...
	if ((reshdl = FindResource(hModule, lpName, RT_RCDATA)) && (hdl = LoadResource(hModule, reshdl)))
	{
		memcpy(&lNum, hdl, 4);
		if (lNum == magic)
		{
...
extern "C" AcRx::AppRetCode acrxEntryPoint(AcRx::AppMsgCode msg, void* appId)
{
	switch (msg)
	{
		case AcRx::kInitAppMsg: acrxUnlockApplication(appId);
#ifdef R15
...
#endif
			break;
		case AcRx::kLoadDwgMsg: gArxInstance = (HINSTANCE)GetModuleHandle(ads_getappname()); funcload();
		...
		case AcRx::kEndMsg: close_console(); CormanLispShutdown();
#ifdef _AFXDLL
			EndMFC();
#endif
			funcunload();
...
int mainx(int argc, char* argv[])
{
	...
	while (1)
	{
		if (PeekMessage(&msg, NULL, 0, 0, TRUE))
		{
			...
			if (msg.message == WM_CHAR && msg.wParam == 3 && (GetKeyState(VK_CONTROL) < 0))
			{
				if (pCormanLisp)
				{
					pCormanLisp->GetNumThreads(&numThreads);
...
BOOL __stdcall HandlerRoutine(DWORD dwCtrlType)
{
	...
	switch (dwCtrlType)
	{
		...
		case CTRL_CLOSE_EVENT: // tja...
			...
			close_console();
			break;
		case CTRL_LOGOFF_EVENT: close_console(); break;
...
#endif
	gInstance = hInstance;
	_cprintf("%s v%0d.%0d.%03d <%s>\r\n", ads_getappname(), VERMAJOR, VERMINOR, BUILD, gImageName);
...
CharBuf::CharBuf(long length) : fCharsAvailable()
{
	fSize = length;
	fBuf = new char[fSize];
	fIndex = 0;
...
long CharBuf::getCharsInBuffer(char* buf, long num)
{
	PLSingleLock myLock(&fSemaphore, TRUE);
	if (fIndex > num)
	{
...
static long FAR PASCAL WndProc(HWND hwnd, UINT message, UINT wParam, LONG lParam)
{
	...
	switch (message)
	{
		case WM_PAINT:
			...
			DrawText(hdc, consoleCaption, -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);

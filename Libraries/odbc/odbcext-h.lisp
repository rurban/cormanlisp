;;;; odbcext-h.lisp -- ODBC EXTRA 3.0 definitions for Corman Lisp

;;;; Author:        Vassili Bykov <vassili@magma.ca>
;;;;                              <vassili@objectpeople.com>
;;;; Created:       11/23/1998

;;;; This is just a no-frills equivalent of SQLEXT.H file for ODBC 3.0
...
/* New defines for SEARCHABLE column in SQLGetTypeInfo */
  ...
  // #if (ODBCVER >= 0x0300)
// VB added: these are defined further in the file:
...
// **VB: the following block is moved from further in the file
...
#LISP (progn (export 'SQL_LEN_DATA_AT_EXEC)
...
#LISP (progn (export 'SQL_LEN_BINARY_ATTR)
	     (defun SQL_LEN_BINARY_ATTR (length)
	       (+ (- length) SQL_LEN_BINARY_ATTR_OFFSET)))

 // **VB** - copieded from ahead in the file
...
#LISP (progn				; UNTESTED!
	...
	(defun SQL_FUNC_EXISTS (pfExists uwAPI)
	  (let ((addr (+ (ct:foreign-ptr-to-int pfExists)
			 (* 4 (ash uwAPI -4)))))
	    (zerop (logand (cref (:long *) (ct:int-to-foreign-ptr addr) 0)
			   (ash 1 (logand uwAPI #xF)))))))
...
#LISP (progn
	(export '(SQL_POSITION_TO SQL_LOCK_RECORD SQL_REFRESH_RECORD
		  SQL_UPDATE_RECORD SQL_DELETE_RECORD SQL_ADD_RECORD))

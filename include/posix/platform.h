...
inline HANDLE FileHandleFromFd(int fd)
{
...
inline int IsFileHandle(HANDLE h)
{
	return (intptr_t)h < -1;
}
...
inline int SetCurrentDirectoryA(const char* dir)
{
	return chdir(dir) == 0;
}
// strncat_s
inline int strncat_s(char* dst, size_t sz, const char* src, size_t cnt)
{
	...
	size_t n = cnt < (sz - dlen - 1) ? cnt : (sz - dlen - 1);
	strncat(dst, src, n);
	return 0;
}
...
{
	return (HMODULE)dlopen(name, RTLD_LAZY);
}
...
{
	return (FARPROC)dlsym((void*)mod, name);
}
inline int FreeLibrary(HMODULE mod)
{
	return dlclose((void*)mod) == 0;
}
...
inline void _ftime_s(struct _timeb* tb)
{
	struct timeval tv;
	gettimeofday(&tv, NULL);
	tb->time = tv.tv_sec;
...
inline int SystemTimeToFileTime(const SYSTEMTIME*, FILETIME*)
{
	return 0;
}
inline int FileTimeToSystemTime(const FILETIME*, SYSTEMTIME*)
{
	return 0;
}
...
{
	pthread_setspecific((pthread_key_t)k, v);
}
...
inline void* HeapReAlloc(HANDLE, DWORD, void* ptr, size_t sz)
{
...
inline int VirtualFree(LPVOID addr, size_t size, DWORD)
{
	return munmap(addr, size) == 0;
}
inline int VirtualProtect(LPVOID addr, size_t size, DWORD np, DWORD*)
{
	return mprotect(addr, size, np) == 0;
}
...
inline int DuplicateHandle(HANDLE, HANDLE, HANDLE, HANDLE*, DWORD, int, DWORD)
{
	return 1;
}
...
#define _beginthread(fn, stack, arg)                          \
	((HANDLE)({                                               \
		pthread_t _t;                                         \
		pthread_create(&_t, NULL, (void* (*)(void*))fn, arg); \
		(unsigned long)_t;                                    \
	}))
...
inline int IsBadReadPtr(const void*, size_t)
{
	return 0;
}
...
inline int OpenProcessToken(HANDLE, DWORD, HANDLE*)
{
	return 0;
}
inline int GetUserProfileDirectoryA(HANDLE, char* buf, DWORD* sz)
{
...
inline int SHGetFolderPathA(HWND, int, HANDLE, DWORD, char* buf)
{
	const char* h = getenv("HOME");
	if (!h)
		h = "/tmp";
...
inline LONG RegSetValue(void*, const char*, DWORD, const char*, DWORD)
{
	return ERROR_SUCCESS;
}
inline LONG RegDeleteKey(void*, const char*)
{
	return ERROR_SUCCESS;
}
...
#define CREATE_ALWAYS 2
inline HANDLE CreateFile(const char* path, DWORD access, DWORD, void*, DWORD creation, DWORD, HANDLE)
{
	int flags = 0;
...
struct IMAGE_SECTION_HEADER
{
	...
	union
	{
		unsigned long PhysicalAddress, VirtualSize;
	} Misc;
